...
namespace TownOfHost
{
    ...
    {
        private static readonly string URL = "https://api.github.com/repos/Paraoneko/TownOfHost-Cat";
        ...
        public static async Task<bool> CheckRelease(bool beta = false, bool all = false, bool forced = false)
        {
            ...
            try
            {
                ...
                {
                    client.DefaultRequestHeaders.Add("User-Agent", "TownOfHost-Cat Updater");
                ...
                else if (all)
                {
                    ...
                    foreach (var release in releases)
                    {
                        ...
                        {
                            if (asset.Name == "TownOfHost-Cat_Steam.dll" && Constants.GetPlatformType() == Platforms.StandaloneSteamPC)
                            {
                                release.DownloadUrl = asset.DownloadUrl;
                                break;
                            }
                            if (asset.Name == "TownOfHost-Cat_Epic.dll" && Constants.GetPlatformType() == Platforms.StandaloneEpicPC)
                            {
                                release.DownloadUrl = asset.DownloadUrl;
                                break;
                            }
                            if (asset.Name == "TownOfHost-Cat.dll")
                                release.DownloadUrl = asset.DownloadUrl;
                        }
                        release.OpenURL = $"https://github.com/Paraoneko/TownOfHost-Cat/releases/tag/{tag}";
                ...
                else
                {
                    ...
                    {
                        if (assets[i]["name"].ToString() == "TownOfHost-Cat_Steam.dll" && Constants.GetPlatformType() == Platforms.StandaloneSteamPC)
                        {
                            downloadUrl = assets[i]["browser_download_url"].ToString();
                            break;
                        }
                        if (assets[i]["name"].ToString() == "TownOfHost-Cat_Epic.dll" && Constants.GetPlatformType() == Platforms.StandaloneEpicPC)
                        {
                            downloadUrl = assets[i]["browser_download_url"].ToString();
                            break;
                        }
                        if (assets[i]["name"].ToString() == "TownOfHost-Cat.dll")
                            downloadUrl = assets[i]["browser_download_url"].ToString();
                    }
        ...
        public static async Task<bool> DownloadDLL(string url, string openurl)
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    using var file = new FileStream("BepInEx/plugins/TownOfHost-Cat.dll", FileMode.Create, FileAccess.Write);
                    stream.CopyTo(file);
        ...
        private static void ShowPopup(string message, bool showButton = false, string OpenURL = "")
        {
            if (InfoPopup != null)
            {
                ...
                if (button != null)
                {
                    ...
                    {
                        Application.OpenURL(OpenURL == "" ? "https://github.com/Paraoneko/TownOfHost-Cat/releases/latest" : OpenURL);
                        Application.Quit();
                    }));
                }
            }
